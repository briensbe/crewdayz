...
export class MonthlyViewComponent implements OnInit, OnDestroy {
  ...
  activeFiltersCount = computed(() => {
    ...
    if (filters.work_site && filters.work_site.length > 0) count++;
    if (filters.contract_type && filters.contract_type.length > 0) count++;
    return count;
  });
  ...
  getCellVal(
    employeeId: string,
    dateStr: string,
  ): { val: string; class: string; comment?: string; absence: Absence; tooltip: string } | null {
    const emp = this.employeeService.employees().find((e) => e.id === employeeId);
    if (emp) {
    ...
    switch (category) {
      ...
      case 'RTT':
        ...
        break;
      case 'Maladie':
        catClass = 'abs-maladie';
        break;
      case 'Congé maternité':
        catClass = 'abs-maternite';
        break;
      case 'Exceptionnel':
        catClass = 'abs-exceptionnel';
        break;
      case 'Formation':
        catClass = 'abs-formation';
        break;
      case 'Temps partiel':
        catClass = 'abs-partiel';
        break;
      case 'Prévisionnel':
        catClass = 'abs-previsionnel';
        break;
    }
  ...
  async onModalDelete(payload: {
    employeeId: string;
    dates: { date: string; period: 'full' | 'morning' | 'afternoon' }[];
  }) {
    this.showModal.set(false);
    try {
      await this.absenceService.deleteEmployeeAbsencesForDates(payload.employeeId, payload.dates, this.year());
    } catch (err: any) {
