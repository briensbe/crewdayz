...
4. **Gestion de l'état dans le service Singleton (`providedIn: 'root'`)** : Stockage du `parseResult`, du `fileName` et du `selectedMonthIndex` directement dans `TriskellReconciliationService` pour préserver l'affichage lors de la navigation entre routes au sein de la session utilisateur.
